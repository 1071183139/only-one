...
class PlaceOrderView(LoginRequiredMixin, View):
    # 1如果用户在商品的详情页面点击直接购买需要向后端传送的参数包括: 商品的sku_id和商品的数量count
    # 2如果用户在购物车页面提交订单，那么需要向后端传送的数据只需要商品的sku_id就行。
    def post(self,request):
        ...
        context = {
            ...
            "sku_ids": ",".join(sku_ids), # 下面生成订单信息的时候会用到
        }
            ...
            with transaction.atomic():
                ...
                try:
                    ...
                    for sku_id in sku_ids:
                        ...
                        GoodsSKU.objects.filter(id=sku_id).\
